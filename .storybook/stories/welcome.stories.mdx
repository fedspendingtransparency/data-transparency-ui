...
import infoGraphic from "../img/ui_ux_approval.png";

<Meta title="Welcome" parameters={{ options: { showAddonPanel: false } }}/>

# Data Transparency User Interface (UI) Component Library
The purpose of this webpage is to give visibility into the patterns built into the 
...
These implementations, refered to as UI Components, exist outside of the USASpending
and Broker codebase.

With this visiblity & independence, the following benefits arise:
...
in a single place and then see those changes propogated throughout the website immediately.
...
The below infographic displays how the UI/UX and Development Teams work together to identify new components for this library.
...
This open-source project has nearly 1K contriubtors and is constantly improving.
...
Storybook supports `mdx` format for stories, we will migrate to that to acheive greater customization over our stories.
...
- Build storybook artifacts and Component's CSS by running `npm run build`
    - Storybook build artifacts are compiled in `docs/` and deployed using GitHub Pages from the master branch
